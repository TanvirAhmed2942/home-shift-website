      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="bg-red-50 p-6 border-b border-red-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-red-700 text-sm">
              Journey exceeds strict 9-hour limit.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="text-sm text-slate-600 leading-relaxed">
            You cannot publish a journey that exceeds 9 hours. What would you
            like to do?
          </div>
...
export function JourneyBuilderPro({
  onSave,
}: {
  ...
  if (
    ...
    metrics.totalDuration > JOURNEY_RULES.WARNING_THRESHOLD_MINUTES
  ) {
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-slate-200 mb-6">
        {draftJourneys.map((journey) => (
          ...
          >
            ...
            <span className="text-xs bg-slate-200 px-1.5 rounded-full text-slate-600">
              {journey.jobs.length}
            </span>
            ...
            {draftJourneys.length > 1 && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (confirm("Delete this journey draft?"))
                    deleteJourneyDraft(journey.id);
                }}
                className={`ml-2 p-0.5 rounded-full hover:bg-red-100 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity ${
                  activeJourneyId === journey.id ? "opacity-100" : ""
                }`}
              >
                <X className="w-3 h-3" />
              </button>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-bold">
                  {activeJourney?.name || "Journey Draft"}
                </h2>
          ...
          <div className="bg-white rounded-2xl shadow-lg border-2 border-dashed border-slate-200 p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">
              Empty Journey Canvas
            </h3>
            <p className="text-slate-500 mb-6 max-w-md mx-auto">
              Select jobs from the "Available Jobs" list to start building this
              route.
            </p>
          </div>
        </div>
            ...
            <div className="lg:col-span-2 bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-lg flex flex-col justify-between relative overflow-hidden">
                ...
                <div
                  className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-2 ${
                    metrics?.isOverLimit
                      ...
                      : metrics?.totalDuration &&
                        metrics.totalDuration >
                          JOURNEY_RULES.WARNING_THRESHOLD_MINUTES
              ...
              <div className="mt-8 relative z-10">
                <div className="flex justify-between text-xs font-semibold text-slate-400 mb-1">
                  <span>
                    Utilization ({Math.round(metrics?.utilizationScore || 0)}%)
                  </span>
                  <span>
                    {Math.floor((metrics?.totalDuration || 0) / 60)}h{" "}
                    {(metrics?.totalDuration || 0) % 60}m / 9h Max
                  </span>
                </div>
                <div className="w-full h-3 bg-slate-700 rounded-full overflow-hidden flex">
                    ...
                    style={{
                      width: `${
                        ((metrics?.totalDrivingTime || 0) /
                          JOURNEY_RULES.MAX_JOURNEY_MINUTES) *
                        100
                      }%`,
                    }}
                  ...
                  <div
                    style={{
                      width: `${
                        ((metrics?.totalBufferTime || 0) /
                          JOURNEY_RULES.MAX_JOURNEY_MINUTES) *
                        100
                      }%`,
                    }}
                    className="bg-slate-500 h-full opacity-50"
                    title="Buffer"
                  ></div>
                </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="text-slate-500 flex items-center gap-2">
                    <Navigation className="w-3 h-3" /> Driving
                  </span>
                  <span className="font-mono font-bold">
                    {Math.floor((metrics?.totalDrivingTime || 0) / 60)}h{" "}
                    {(metrics?.totalDrivingTime || 0) % 60}m
                  </span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="text-slate-500 flex items-center gap-2">
                    <Package className="w-3 h-3" /> Loading
                  </span>
                  <span className="font-mono font-bold">
                    {Math.floor((metrics?.totalLoadingTime || 0) / 60)}h{" "}
                    {(metrics?.totalLoadingTime || 0) % 60}m
                  </span>
                </div>
                <div className="flex justify-between items-center p-2 bg-slate-50 rounded-lg">
                  <span className="text-slate-500 flex items-center gap-2">
                    <Coffee className="w-3 h-3" /> Buffer
                  </span>
                  <span className="font-mono font-bold text-slate-400">
                    {Math.floor((metrics?.totalBufferTime || 0) / 60)}h{" "}
                    {(metrics?.totalBufferTime || 0) % 60}m
                  </span>
                </div>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex flex-col items-center gap-1 mt-1">
                      <div
                        className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm cursor-move ${
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-mono text-slate-500 font-bold bg-slate-100 px-1.5 rounded">
                          {stop.calculatedArrival?.toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </span>
                      ...
                      {stop.distanceToNext > 0 && (
                        <div className="flex items-center gap-2 text-xs text-slate-400 mt-2 pt-2 border-t border-slate-100">
                          ...
                          <span>
                            {Math.round(stop.drivingToNext || 0)}min drive
                          </span>
                        </div>
                      )}
                    </div>
